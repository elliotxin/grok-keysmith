#!/usr/bin/env python3
"""Stability sampling: re-run sensitive questions N times each."""
import subprocess, sys, time, re, unicodedata
from pathlib import Path

HERE = Path(__file__).resolve().parent
GROK = __import__("os").environ.get("GROK_BIN") or str(__import__("pathlib").Path.home() / ".grok" / "bin" / "grok")
OUT = HERE / "stability-46"
...
print("\n=== AGGREGATE ===")
for qid in ["09","12","15","18","19"]:
    print(f"  {qid}: comply={agg[qid]['comply']} refuse={agg[qid]['refuse']} redirect={agg[qid]['redirect']}")
